...
| 5.4 | LLM calls routed through Bedrock via OAuth/API GW proxy | ✅ PASS | LLMGenerate pattern confirmed working 2026-05-14: SSO → Secrets Manager → Azure AD OAuth2 → API GW proxy → Bedrock; `llm_gen.py` in `AiDa/AiDa-ADO/aid_aida-backend/Bedrock_calls/`; `aida_proxy_client.py` shares same ARNs (inference profile `1lgjlvmziewi`, guardrail `vfscnyuxyy9a`, API GW `cpvmmdvf08`). Direct `boto3.converse_stream()` still IAM-blocked — use proxy path. |
...
| 5 — AiDa / Bedrock Onboarding | 5 | 2 | 0 | 3 |
| 6 — AD Groups | 2 | 0 | 0 | 2 |
...
**D1 infrastructure/governance ACs (Features 5–9): 2/15 PASS, 1 partial** (remaining partial: AC 8.1 Architecture diagram)  
